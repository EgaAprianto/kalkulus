...
const Simpan1 = () => {
    ...
    // simpan
    const [simpan1, setSimpan1] = useState(() => {
      const storedData = localStorage.getItem('Simpan1');
      return storedData ? JSON.parse(storedData) : NaN;
    });

    //Nilai Hasil
    const [hasiljumlahI, setHasilJumlahI] = useState(isNaN(simpan1) ? 0 : simpan1);
    const [hasiljumlahJ, setHasilJumlahJ] = useState(isNaN(simpan1) ? 0 : simpan1);
    const [hasiljumlahK, setHasilJumlahK] = useState(isNaN(simpan1) ? 0 : simpan1);
    ...
    let simpanData = (e) => {
      ...
      setSimpan1(dataSimpan1)
    };

    useEffect(() => {
      localStorage.setItem('Simpan1', JSON.stringify(simpan1));
    }, [simpan1]);

    useEffect(() => {
      const storedData = localStorage.getItem('Simpan1');
      if (storedData) {
        setSimpan1(JSON.parse(storedData));
      }
    }, []);

    useEffect(() => {
      if (simpan1) {
        setHasilJumlahI(simpan1.Hasil);
        setHasilJumlahJ(simpan1.hasiljumlahJ);
        setHasilJumlahK(simpan1.hasiljumlahK);
        setI1(simpan1.Inputan);
        setI2(simpan1.i2);
        setj1(simpan1.j1);
        setj2(simpan1.j2);
        setK1(simpan1.k1);
